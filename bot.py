...
def set_user_city(message):
    ...
    dbworker.update_user_current_state(message.chat.id, config.States.S_ENTER_CITY.value)

@bot.message_handler(func=lambda message: dbworker.get_user_current_state(message.chat.id) == config.States.S_ENTER_CITY.value)
def enter_city(message):
    ...
    dbworker.update_user_current_state(message.chat.id, config.States.S_CITY_SETTED.value)
    main_menu(message.chat.id)


@bot.message_handler(func=lambda message: 
    dbworker.get_user_current_state(message.chat.id) == config.States.S_CITY_SETTED.value 
    and message.text == "🌤 Current Weather"
)
def get_current_weather_report(message):
...
def get_five_day_weather_report(message):
    city_name = dbworker.get_user_city_name(message.chat.id)
